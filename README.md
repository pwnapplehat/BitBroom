...
| **Duplicates** | Content-verified duplicate finder (size → head hash → full SHA-256, so it never lies about a match), an empty-folder finder, and a **Dev junk** mode that finds regenerable build folders (`node_modules`, `target`, `.venv`, `dist`, `.next`, …) — only when they sit next to a real project manifest. Recycle Bin-only deletion, and one copy of every duplicate group always survives — enforced in the engine, not just the UI. |
