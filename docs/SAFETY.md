...
| Analyzer / Duplicates / Empty-folder manual deletions | `SHFileOperation` with `FOF_ALLOWUNDO` → **always Recycle Bin**, gated by `ManualDeleteGuard`, which refuses drive roots, the Windows/Program Files/ProgramData/Users/profile-root trees, drive-root system items — **and installed-application trees** (`RuntimeAppGuard`: per-user program installs under `%LOCALAPPDATA%\Programs`, plus any tree carrying Electron/Squirrel/`.asar` markers found by walking parents). Installed apps ship the same layout as user data / dev projects, so this stops a duplicate DLL or an app's own empty folder from being recycled out from under it. |
...
| Dev junk deletions | **Always Recycle Bin.** A folder is only ever offered when its name matches a known artifact (`node_modules`, `target`, `.venv`, …) **and** the project manifest proving its context (`package.json`, `Cargo.toml`, `pyvenv.cfg`, …) is present — checked at scan time and re-checked immediately before recycling. It also **refuses runtime-app locations**, because installed software ships the same layout as a dev project (an Electron app is `package.json` + `out`/`node_modules`): anything under `AppData` (except `%TEMP%`), `Program Files`, `ProgramData`, `Windows`; dot-folders directly under the profile (`.cursor`, `.vscode`…); and any tree carrying app-runtime markers (`icudtl.dat`, `v8_context_snapshot.bin`, Squirrel `Update.exe`, `*.asar`) found by walking outward through parent folders. Matched artifacts are never recursed into; junctions never followed. |
| WSL/Docker disk compaction | best-effort `fstrim` inside each distro, `wsl --shutdown`, then `diskpart` `attach vdisk readonly` + `compact vdisk` — the read-only attach enables the zero-block scan (same mechanism as `Optimize-VHD -Mode Full`) and by definition cannot modify the guest filesystem; compaction only releases blocks the guest already freed. On any failure a separate detach pass always runs, so a disk is never left attached. Works on every Windows edition (no Hyper-V needed). |
| OneDrive free-up-space | `attrib +U -P` — the documented pin/unpin attributes behind Explorer's own "Free up space". Files are dehydrated to online-only, **never deleted**; the cloud copy is untouched and re-downloads on open. |
| Old-driver removal | `pnputil /delete-driver` **without** `/force` or `/uninstall` — Windows itself refuses any package still bound to a device. BitBroom only offers *strictly older* versions of a driver family whose newest version stays staged; same-version duplicates and unique drivers are never candidates. |
